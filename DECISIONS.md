...
## D-011：匯入驗證、設定分離強化、背景嗶聲抑制（2026-05-25 cross-review 修正）

**問題**：cross-review（Codex）指出三個與文件不一致或健壯性不足的行為。

| # | 問題 | 修正 |
|---|------|------|
| 1 | 匯入 JSON 無 schema 驗證（day 範圍、時間格式、筆數） | `importScheduleJSON()` 改為：筆數超過 `MAX_IMPORT_EVENTS`(1000) 直接拒絕；逐筆驗證 `day` 為 0–6 整數、起訖時間符合 `HH:MM`（`normalizeTimeStr()`），不合格者略過並回報略過筆數；全部不合格則整體失敗、不動現有資料 |
| 2 | 匯入會覆蓋 `title` 設定，與 D-010「設定與排程分開」矛盾 | 移除匯入時寫入 `settings.title` 的邏輯。匯出 JSON 仍附帶 title（供記錄），但匯入不再覆蓋個人設定 |
| 3 | README 稱背景「不補發嗶聲」，但 resume 後可能補發延遲嗶聲 | `updateCountdowns()` 加入 `ALERT_GRACE_MS`(5s)：活動結束時間距現在超過 grace 才視為「背景錯過」，不觸發 `triggerAlert`，使行為符合文件描述 |

**原因**：匯入是唯一的外部資料入口，需防呆（壞檔卡瀏覽器 / 塞爆 localStorage）；title 覆蓋違反既有 D-010 決策；延遲嗶聲是非預期提醒，使用者回前景被早已結束的活動嚇到不合理。

**安全備註**：所有匯入欄位都經型別檢查後才存入；渲染全程使用 `textContent`（無 `innerHTML` 注入），匯入資料不會進入程式執行 context。

**附帶修正**：`importScheduleJSON()` 改為回傳結果物件，修掉舊版「匯入失敗仍 alert 成功」的 bug（呼叫端不再無條件顯示成功）。
